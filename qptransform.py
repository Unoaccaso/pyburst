...
DEFAULT_FRANGE = (0.0, float("inf"))
DEFAULT_QRANGE = (2.0 * numpy.pi, 20.0 * 2.0 * numpy.pi)
DEFAULT_PRANGE = None
DEFAULT_ALPHA = 1.0
DEFAULT_MAX_Qp_ITERS = 100
DEFAULT_MAX_BASINHOPPING_ITERS = 5
NUMBER_OF_SIGMAS = 3.0
DEFAULT_ENERGY_DENSITY_THRESHOLD = 7.0
...
class QpTile(QpObject):
    ...
    @property
    def sigma_tau(self):
        return self.Q / (4.0 * numpy.pi * self.frequency)
    ...
    def ntiles(self):
        ...
        """
        n_tiles = round_to_power(
            self.duration / (self.alpha * self.sigma_tau), base=2, which="upper"
        )
        while (
            0.5 * (n_tiles / self.duration)
            < self.frequency + NUMBER_OF_SIGMAS * self.sigma_phi + 1.0 / self.duration
        ):
            n_tiles *= 2
        return n_tiles
    ...
    def get_data_indices(self):
        ...
        return numpy.round(self._get_indices() + self.frequency * self.duration).astype(
            int
        )

    def get_window(self, fft_freqs):
        """Generate the Gaussian window for this row with 'frequency' as central frequency"""
        Qtilde = self.Q / numpy.sqrt(1.0 + 2.0j * self.Q * self.p)
        norm = (
            ((2.0 * numpy.pi / (self.Q**2)) ** (1.0 / 4.0))
            * numpy.sqrt(1.0 / (2.0 * numpy.pi * self.frequency))
            * Qtilde
        )
        exp_argument = (
            (0.5 * Qtilde * (fft_freqs - self.frequency)) / self.frequency
        ) ** 2
    ...
    def transform(
        ...
        if original_sampling_rate == False:
            ...
            if derivative is True:
                fft_series *= 2.0 * numpy.pi * 1.0j * fft_freqs
            ...
            transform_ts = numpy.sqrt(self.sampling) * TimeSeries(
                transform, x0=start_time, dx=self.duration / transform.size, copy=False
            )

            # here we properly shift in frequency
            transform_ts *= numpy.exp(
                -2.0j * numpy.pi * fft_freqs[0] * transform_ts.times.value
            )
...
class QpPlane(QpObject):
    ...
    def __init__(self, frange, Q, p, duration, sampling, alpha=DEFAULT_ALPHA):
        ...
        lowest_phi = (1.0 / self.duration) * (
            1.0
            - (NUMBER_OF_SIGMAS * numpy.sqrt(1 + ((2 * self.p * self.Q) ** 2))) / self.Q
        )
        highest_phi = (self.sampling / 2.0) / (
            1.0
            + (NUMBER_OF_SIGMAS * numpy.sqrt(1 + ((2 * self.p * self.Q) ** 2))) / self.Q
        )  # no integration above sampling=2*(frange[-1]+(NUMBER_OF_SIGMAS*numpy.sqrt(1+((2*self.p*self.Q)**2)))*sigma_phi(phi=frange[-1]))
    ...
    def frequencies(self):
        ...
        nfreq = int(
            numpy.ceil(
                numpy.log(self.frange[1] / self.frange[0])
                / numpy.log(
                    1.0
                    + (self.alpha * numpy.sqrt(1 + ((2 * self.p * self.Q) ** 2)))
                    / self.Q
                )
            )
        )
    ...
    def transform(
        ...
        fft_freqs,
        start_time,
        energy_density_threshold=DEFAULT_ENERGY_DENSITY_THRESHOLD,
        derivative=False,
        original_sampling_rate=False,
    ):
        """
        ...
        return QpGram(
            ...
            sampling=self.sampling,
            alpha=self.alpha,
            energy_density_threshold=energy_density_threshold,
        )
...
class QpGram(QpObject):
    ...
    def __init__(
        ...
        self.weighted_energy_density_threshold = (
            self.energy_density_threshold * numpy.average(self.frequency_weight())
        )
    ...
    def find_energy_density(self):
        ...
        for energy in self.energies:
            ...
            energy[energy < self.energy_density_threshold] = 0.0
            area[energy < self.energy_density_threshold] = 0.0
            times = numpy.linspace(0.0, self.duration, len(energy))
            energy_integral_array[item] = integrate.trapezoid(
                energy, times
            )  # use trapezoid for avoiding that in iterpolating zero and non zero it takes negative values
    ...
    def frequency_weight(self):
        ...
        )  # numpy.sqrt(self.frequencies/self.frequencies[-1])
    ...
    def find_weighted_energy_density(self):
        ...
        for energy in self.energies:
            energy = energy.value.copy()
            area = numpy.ones(len(energy))
            energy[energy < self.weighted_energy_density_threshold] = 0.0
            area[energy < self.weighted_energy_density_threshold] = 0.0
            times = numpy.linspace(0.0, self.duration, len(energy))
            energy_integral_array[item] = weight[item] * integrate.trapezoid(
                energy, times
            )  # use trapezoid for avoiding that in iterpolating zero and non zero it takes negative values
            area_integral_array[item] = integrate.trapezoid(area, times)
            item += 1
...
class QpTransform:
    ...
    def __init__(
        ...
        alpha=DEFAULT_ALPHA,
        alpha_find_Qp=DEFAULT_ALPHA,
        energy_density_threshold=DEFAULT_ENERGY_DENSITY_THRESHOLD,
        Qp_finder="energy_density",
        qrange=DEFAULT_QRANGE,
        prange=DEFAULT_PRANGE,
        max_Qp_iters=DEFAULT_MAX_Qp_ITERS,
        filmethod="highpass_threshold_filtering",
        denoising_threshold=DEFAULT_ENERGY_DENSITY_THRESHOLD,
    ):
        ...
        if derivative == True:
            ...
            fft_freqs = (
                scipy.fft.fftfreq(len(fft_series)) * self.series.sample_rate.value
            )
            self.qpgram_derivative = QpPlane(
                Q=self.Q,
                p=self.p,
                frange=frange,
                duration=self.series.duration.value,
                sampling=self.series.sample_rate.value,
                alpha=alpha,
            ).transform(
                fft_series,
                fft_freqs,
                self.series.times.value[0],
                energy_density_threshold,
                derivative=derivative,
            )
        ...
        self.qpspecgram = qpgram_to_qpspecgram(
            qp_scan(
                ...
                prange=[self.p, self.p],
                max_Qp_iters=max_Qp_iters,
                original_sampling_rate=True,
            ),
            to_return="energies",
        )

        if filmethod is not None:
            self.filseries = iQpt_filtering(
                self.series, self.times, self.qpspecgram, filmethod, denoising_threshold
            )
...
def find_Q(Q, *args):
    ...
    qpgram = QpPlane(
        Q=Q, p=p, frange=frange, duration=duration, sampling=sampling, alpha=alpha
    ).transform(fft_series, fft_freqs, start_time, energy_density_threshold)
...
def qp_scan(
    ...
    prange=DEFAULT_PRANGE,
    max_Qp_iters=DEFAULT_MAX_Qp_ITERS,
    max_basinhopping_iters=DEFAULT_MAX_BASINHOPPING_ITERS,
    original_sampling_rate=False,
):
    """
    ...
    if qrange[0] == qrange[-1] and prange[0] == prange[-1]:
        ...
        if prange == [0.0, 0.0]:
            ...
            minimizer_kwargs = {
                "method": DEFAULT_MINIMIZE_METHOD,
                "args": (
                    0.0,
                    frange,
                    duration,
                ...
                "bounds": Q_bnds,
                "options": {"maxiter": max_Qp_iters},
            }
            res_Q = basinhopping(
                find_Q,
                x0=numpy.sqrt(qrange[-1] * qrange[0]),
                minimizer_kwargs=minimizer_kwargs,
                niter=max_basinhopping_iters,
            )
            if res_Q.fun == 0.0 or res_Q.success == False:
                warnings.warn(
                    "'energy_density' does not converge for finding (Q,p), 'energy_peak' is used instead"
                )
            ...
            if prange is not None:
                Qp_bnds = scipy.optimize.Bounds(
                    lb=[qrange[0], prange[0]], ub=[qrange[-1], prange[-1]]
                )
            elif prange is None:
                Qp_bnds = scipy.optimize.Bounds(
                    lb=[qrange[0], 0], ub=[qrange[-1], 1.0 / qrange[0]]
                )  # pensaci
            # to_return='energy_density'
            ...
            res_Qp = basinhopping(
                find_Qp,
                x0=[numpy.sqrt(qrange[-1] * qrange[0]), 0.0],
                minimizer_kwargs=minimizer_kwargs,
                niter=max_basinhopping_iters,
            )

            if res_Qp.fun == 0.0 or res_Qp.success == False:
                warnings.warn(
                    "'energy_density' does not converge for finding (Q,p), 'energy_peak' is used instead"
                )
                to_return = "energy_peak"
...
def qpgram_to_qpspecgram(qpgram, to_return):
    ...
    if to_return == "energies":  # energies for Spectrogram
        ...
        to_return == "plane"
    ):  # plane for specific purposes (e.g. interpolate likelihood)
    ...
    ny = frequencies.size
    qpmatrix = Spectrogram(
        numpy.empty((nx, ny), dtype=dtype),
        times=qpobject[0].times,
...
def qpobject_filtering(
    reference_qpobject, application_qpobject, filmethod, denoising_threshold
):
    """
    ...
    if filmethod == "highpass_threshold_filtering":
        ...
        if len(denoising_threshold) != 2:
            warnings.warn(
                "For range pass threshold filtering threshold should have two elements, original 'application_qpobject' is returned"
            )
        ...
        warnings.warn(
            "'filmethod' should be 'highpass_threshold_filtering', 'lowpass_threshold_filtering' or 'rangepass_threshold_filtering', original 'application_qpobject' is returned"
        )
        return application_qpobject
    ...
    filqpobject.value[boolean_qpobject] = 0.0
...
def filfreqboundaries(filqpobject):
    ...
    for i in range(len(times)):
        i_lst = [times[i]]
        if numpy.linalg.norm(filqpobject[i, :]) == 0.0:
...
def iQpt_filtering(series, times, qpobject, filmethod, denoising_threshold):
    ...
    f_s = numpy.ceil(1.0 / (times[1] - times[0]))  # Hz
    frequencies = scipy.fft.fftfreq(len(times)) * f_s

    fft_series = scipy.fft.fft(series)

    filqpobject = qpobject_filtering(
        reference_qpobject=qpobject,
        application_qpobject=qpobject,
        filmethod=filmethod,
        denoising_threshold=denoising_threshold,
    )
    freqboundaries = filfreqboundaries(filqpobject)

    Qtilde = qpobject.Q / (numpy.sqrt(1 + 1.0j * 2.0 * qpobject.p * qpobject.Q))
    ...
    for i in range(len(fil_series)):
        if len(freqboundaries[i]) > 1:
            ...
            for k in range(1, len(freqboundaries[i])):
                window += ((-1.0) ** (k - 1)) * special.erf(
                    (Qtilde / 2.0) * ((frequencies / freqboundaries[i][k]) - 1.0)
                )
            window = window / numpy.real(special.erf(Qtilde / 2.0))
...
def inverse_Qp_transform(qpmatrix_derivatives, times):
    ...
    Qtilde = qpmatrix_derivatives.Q / (
        numpy.sqrt(1 + 1.0j * 2.0 * qpmatrix_derivatives.p * qpmatrix_derivatives.Q)
    )

    norm = numpy.sqrt(times[1] - times[0]) / (
        ((2.0 / ((qpmatrix_derivatives.Q**2) * numpy.pi)) ** (1.0 / 4.0))
        * numpy.real(special.erf(Qtilde / 2.0))
    )

    iqpt_value = numpy.zeros(len(times), dtype=float)

    for i in range(len(times)):
        iqpt_value[i] = numpy.real(
            integrate.simpson(
                qpmatrix_derivatives[i, :].value
                / (1.0j * ((2.0 * numpy.pi * frequencies) ** (3.0 / 2.0))),
                frequencies,
            )
        )

    iqpt_value = 2.0 * norm * iqpt_value
