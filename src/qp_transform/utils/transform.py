...
__global__ void qp_transform(
    ...
    if ((phi_idx < n_rows) && (fft_freq_idx < n_cols)){
        ...
        // wavelet parameters
        const float real_part = 1.0;
        const float imag_part = 2.0 * Q * p;
        const complex<float> q_tilde = Q / sqrt(complex<float>(real_part, imag_part));
        ...
        // the actual wavelet
        const complex<float> wavelet = qp_norm * exp(- qp_exponent_2);

        // scalar product and normalization for fft
        const float fft_normalization_factor = sqrtf(sampling_rate);
        const complex<float> out_pixel = fft_value * wavelet * fft_normalization_factor;
        fft_phi_norm_plane[phi_idx * n_cols + fft_freq_idx] = out_pixel;
    }
}
""",
    "qp_transform",
